...
GPT Image 2 프롬프트 설계, 역프롬프팅, 이미지 편집 워크플로우, API 사용 가이드, Codex `image_generation` 연동을 위한 Codex 스킬입니다.

이 저장소는 **이미지 모델 자체**도 아니고, **독립 실행형 이미지 생성 서비스**도 아닙니다. Codex가 막연한 이미지 아이디어, 레퍼런스 이미지, 구현 요청을 구조화된 GPT Image 2 프롬프트와 실제 작업 계획으로 바꿀 수 있도록 돕는 스킬입니다.

## 이 스킬의 용도

이 스킬은 다음 작업에 적합합니다.

- GPT Image 2용 고품질 프롬프트 작성
- 레퍼런스 이미지를 재현 가능한 프롬프트로 역프롬프팅
- 유지할 요소와 변경할 요소를 분리한 이미지 편집 프롬프트 작성
- 시네마틱 포스터, 인물 사진, 제품 사진, 광고, 썸네일, UI 목업, 인포그래픽, 캐릭터 콘셉트 제작
- `gpt-image-2` API 구현 가이드 작성
- Codex / Responses API `image_generation` 워크플로우 설계
- 프롬프트 검수, 실패 원인 분석, 반복 개선

## 핵심 아이디어

이 스킬은 프롬프트 감산 실험에서 얻은 구조를 바탕으로, 이미지를 통제 가능한 슬롯으로 나누어 설계합니다.
...
이 구조는 포스터, 썸네일, 상세페이지, 제품 광고, 텍스트가 들어가는 이미지처럼 레이아웃 통제가 중요한 작업에 특히 유용합니다.
...
```text
$gpt-image-2로 제품 광고 이미지를 생성하고 검증하는 Python API 워크플로우를 작성해줘.
```
...
- 유지할 것: 인물 정체성, 포즈, 제품 형태, 레이아웃, 카메라 각도 등
- 변경할 것: 배경, 의상, 색감, 스타일, 오브젝트 배치, 텍스트 등
- 방지할 것: 얼굴 변화, 손 왜곡, 로고 추가, 텍스트 깨짐, 레이아웃 붕괴 등

이렇게 나누면 Codex가 더 명확한 편집 지시문을 만들고, 결과 검수도 쉬워집니다.

## API 및 Codex 경로

이 스킬은 두 가지 구현 경로를 다룹니다.

- Image API에서 `model="gpt-image-2"`를 직접 지정하는 방식
- Responses API / Codex 워크플로우에서 `image_generation` 도구를 사용하는 방식

자세한 내용은 [references/api-and-codex-routes.md](references/api-and-codex-routes.md)를 참고하세요.
...
- API 작업에서 모델을 확정하고 싶다면 `model="gpt-image-2"`를 명시하세요.
- 역프롬프팅은 원본 프롬프트 복원이 아니라 재현 가능한 프롬프트 설계입니다.
- 이 저장소에는 원본 PDF, Notion 전문, 다른 repo의 프롬프트 컬렉션 원문을 포함하지 않습니다.
- 실제 인물, 브랜드, 로고, 저작권 레퍼런스를 사용할 때는 관련 권리와 정책을 확인해야 합니다.

## 출처

이 스킬은 OpenAI 공식 문서, 프롬프트 감산 실험, 공개 GPT Image 2 커뮤니티 워크플로우에서 얻은 패턴을 요약·재구성한 것입니다. 요약 메모는 [references/source-notes.md](references/source-notes.md)에 정리되어 있습니다.
