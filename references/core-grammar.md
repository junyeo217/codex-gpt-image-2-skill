...
화이트리스트 네거티브는 동결 문장 내부 구성 요소로만 존재하며, 동결 문장 밖에서는 어떤 네거티브도 허용되지 않는다(Tier-0 긍정 재작성 사용). 즉 `no logo` · `no watermark` 같은 어휘를 프롬프트 다른 위치(예: 별도의 Scene 절)에 독립적으로 써도 되는 것이 아니다 — 정본 동결 문장이 본문 어딘가에 있다는 사실이 그 문장 밖의 같은 어휘를 면제해주지 않는다.
